...
export default class App extends Component{
  ...
  componentDidMount=()=>{
    ...
    firebase.firestore().collection("business_data").doc(c[1]).collection("open_orders").doc(c[2]).collection("updates").orderBy("date").onSnapshot(querySnapshot => {
      const groups = [];
      ...
      firebase.firestore().collection("business_data").doc(c[1]).collection("open_orders").doc(c[2]).onSnapshot(doc => { this.setState({customer:doc.data()}) })
      firebase.firestore().collection("business_data").doc(c[1]).onSnapshot(doc => { this.setState({business:doc.data()}) })
...
onSignInSubmit = async(e) => {
  ...
  let p = this.state.customer.customer_number

  let cn = this.state.customer.customer_name

  let ut = this.state.uploadtext

  console.log("props =====>   " + this.props.access_code)
          var c = this.props.access_code.split("/")
  ...
  console.log(e);
  this.setUpRecaptcha();
  let phoneNumber = this.state.customer.customer_number.split(" ").join("").slice(-10)
  ...
  firebase
    ...
    .then(function (confirmationResult) {
      ...
      console.log("inside then right now");
      ...
      var code = window.prompt(`Enter OTP sent to ${p}\n\nOTP verification is mandatory to verify authenticity`)
     ...
     .then(function (result) { 
          ...
          firebase.firestore().collection("business_data").doc(c[1]).collection("open_orders").doc(c[2]).collection("updates").add({by:"customer", update_msg_subject:`New Message by ${cn}` ,update_msg :ut, date : new Date().valueOf()  })
    ...
    this.setState({code_sent:false,uploadtext:""})
};
...
<body>
        ...
        <h4 style={{color:"#000",fontWeight:200,fontSize:35,textAlign:'center',marginTop:0,fontFamily: "-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif"}}>
            
              

                  <img alt="Responsive image" src={ this.state.business.logo} style={{ maxWidth: "100%",height:"auto",borderRadius:0}}  />
            ...
            <h4 style={{color:"#262626",textAlign:"center",borderRadius:-6,backgroundColor:"#eee",padding:4,marginLeft:0,fontWeight:200,fontSize:40,textTransform:"capitalize",marginTop:-1.5,marginBottom:15,}}>
                ...
                { this.state.code_sent 
                  ...
                  ?<div>
                        ...
                        <h4 style={{color:"#ff0000",textAlign:"left",borderRadius:-6,padding:4,marginLeft:10,fontWeight:700,fontSize:16,marginTop:-5.5,marginBottom:15,}}>
                  
                          Please Wait, OTP verification is mandatory to send a message to {this.state.business.name}
                ...
                <h4 style={{color:"#262626",textAlign:"center",borderRadius:-6,backgroundColor:"#eee",padding:4,marginLeft:0,fontWeight:200,fontSize:40,textTransform:"capitalize",marginTop:40.5,marginBottom:15,}}>
        ...
        <footer class={"card row"} style={{position:"fixed" ,textAlign:"center", bottom:"10px",paddingTop:10,paddingBottom:-30,margin:-10,height:90,backgroundColor:"#262626",opacity:0.9, width: "105%",borderRadius:0}} >
               ...
               <a href={`whatsapp://send?text=Hello,\n\nJust visited my order page, wanted to know more.\n\n\nOrder Status URL : https://statusapp-19926.web.app/${c[1]}/${c[2]}&phone=${this.state.business.phonenumber}`} target="_blank" style={{ marginLeft:"2%",textAlign:"center",backgroundColor:"#eee",borderRadius:6,width:"30%",backgroundColor:"#33cc33",paddingTop:15,paddingBottom:5}}>
